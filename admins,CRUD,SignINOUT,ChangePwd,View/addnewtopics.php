...
if(isset($_POST['topic']))
{
    try
    {
        ...
        $stmt=$con->prepare("select * from topics where tname=:tn  and of_client=:cid");
        $stmt->bindValue(':tn',strtoupper($_POST['topic']));
        $stmt->bindValue(":cid",$_SESSION['uid'],PDO::PARAM_INT);
        $stmt->execute();
        $recs=$stmt->fetchAll();
        if(count($recs)==0)
        {
            //topic not availabe so add it to table
            $stmt1=$con->prepare("insert into topics(tname, of_client) values(:tn,:cid)");
            $stmt1->bindValue(":tn",strtoupper($_POST['topic']));
            $stmt1->bindValue(":cid",$_SESSION['uid'],PDO::PARAM_INT);
            $stmt1->execute();
